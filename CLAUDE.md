...
When the user sends their first message of a session, run these three checks
before responding. Use only built-in tools (Read, Grep, Glob) to avoid
permission prompts. Get today's date from `currentDate` in system context.

### 1. Pending context updates
Check if `memory/pending-updates.md` exists and has content. If it does,
show the pending entries and ask:
> "The daily brief cron logged [N] pending context updates. Want me to apply them?"
Walk through each entry. After applying (or skipping), remove processed entries.

### 2. Vacation return check
Read `work/vacation.md`. If Status is `active` and today is after the End
date, prompt:
> "Welcome back! You were on PTO from [start] to [end]. Want me to generate
> your return brief now?"
If yes, run `prompts/return-brief.md`, then set Status to `inactive`.

### 3. Weekly reprioritization (Mon/Fri only)
Check the `_Last updated` date in `work/priorities.md`. If it is Monday
morning or Friday afternoon, and the last update was more than 6 days ago,
prompt:
...
### Output format
If nothing needs attention, say so in one line and move on.

For a full staleness sweep, open thread scan, and relationship recency
check, run `prompts/context-audit.md` on demand.
...
- `prompts/context-audit.md` -- full staleness sweep, open threads, relationship recency
